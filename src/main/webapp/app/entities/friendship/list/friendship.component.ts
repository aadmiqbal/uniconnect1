...
export class FriendshipComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFriendship): number => this.friendshipService.getFriendshipIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.friendships = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFriendship[] | null): IFriendship[] {
    const friendshipsNew = this.friendships ?? [];
    if (data) {
      for (const d of data) {
        if (friendshipsNew.map(op => op.id).indexOf(d.id) === -1) {
          friendshipsNew.push(d);
        }
      }
    }
    return friendshipsNew;
  }
